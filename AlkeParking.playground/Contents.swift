...
struct Parking {
    ...
    var parkingEarnings:(earnings: Int,vehicleCheckOut: Int) = (0, 0)
    ...
    mutating func checkOutVehicle(plate: String, onSucess: (Int) -> Void, onError: () -> Void) {
        if let vehicle = vehicles.first(where: { vehicle in vehicle.plate == plate }) {
            ...
            onSucess(pricePaid)
            parkingEarnings.earnings += pricePaid
            vehicles.remove(vehicle)
            parkingEarnings.vehicleCheckOut += 1
        } else {
            onError()
        }
    }
    ...
    func calculateFee(type:VehicleType, parkedTime:Int, hasDiscountCard: Bool) -> Int {
        ...
        if parkedTime > 120 {
            ...
            price += Int(floor(priceCalculate))*5
        }
    ...
    func earningAndExitVehicles(){
        print("\(parkingEarnings.vehicleCheckOut) vehicles have checked out and have earnings of $\(parkingEarnings.earnings)")
    }
    
    func listVehicles(){
        for (index, vehicle) in vehicles.enumerated() {
            print("\(index+1) - \(vehicle.plate)")
        }
    }
...
let vehicle7 = Vehicle(plate: "OO333CC", type: .MiniBus, checkInTime: Date(), discountCard: nil)
...
let vehicle11 = Vehicle(plate: "TC333EE", type: .MiniBus, checkInTime: Date(), discountCard: nil)
...
let vehicle16 = Vehicle(plate: "CA111AA", type: .Car, checkInTime: Date(), discountCard: "DISCOUNT_CARD_001")
let vehicle17 = Vehicle(plate: "Q222BBB", type: .Moto, checkInTime: Date(), discountCard: nil)
let vehicle18 = Vehicle(plate: "QC333CC", type: .MiniBus, checkInTime: Date(), discountCard: nil)
let vehicle19 = Vehicle(plate: "AB444DD", type: .Bus, checkInTime: Date(), discountCard: "DISCOUNT_CARD_002")
let vehicle20 = Vehicle(plate: "KA111AA", type: .Bus, checkInTime: Date(), discountCard: "DISCOUNT_CARD_002")
let vehicle21 = Vehicle(plate: "AY111AA", type: .Bus, checkInTime: Date(), discountCard: "DISCOUNT_CARD_002")
...
var vehicles = [vehicle1, vehicle2, vehicle3, vehicle4, vehicle5, vehicle6, vehicle7, vehicle8, vehicle9, vehicle10,vehicle11, vehicle12, vehicle13, vehicle14, vehicle15, vehicle16, vehicle17, vehicle18, vehicle19, vehicle20, vehicle21]
...
alkeParking.checkOutVehicle(plate: "B222BBB", onSucess: { parkingPrice in
    print("Your fee is $\(parkingPrice). Come back soon")
}) {
...
alkeParking.earningAndExitVehicles()
